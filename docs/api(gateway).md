# 接口文档(网关方式)

[toc]

## 网关

请求会先经过网关，在网关进行一次解析之后再到对应的资源地址。

```http
http://gateway.cdeledu.com/doorman/op
```

通过网关方式的新接口请求如无特别说明都将以POST方式发送JSON格式数据来进行。

## 通用参数和通用返回

请求：

```json
{
    "params": "--业务参数加密串--",
    "domain": "chinaacc",
    "resourcePath": "+/dlclassroom/getMarqueeInfoListGateway",
    "platform": "",
    "appType": "",
    "appKey": "",
    "appVersion": "",
    "publicKey": "--公钥--",
    "aesKey": "--加密串--",
    "sid": ""
}
```

请求参数：

| 字段         | 说明              | 备注 |
| ------------ | ----------------- | ---- |
| params       | 业务参数加密串    | 必选 |
| domain       | 资源所在域标识    | 必选 |
| resourcePath | 资源路径标识      | 必选 |
| platform     | 来源平台          | 可选 |
| appType      | 来源app类型和版本 | 可选 |
| appKey       | App唯一标识       | 可选 |
| appVersion   | 来源app类型和版本 | 可选 |
| publicKey    | 公钥              | 必选 |
| aesKey       | 加密后的密钥      | 必选 |
| sid          | portal的sid       | 可选 |

返回：

```json
{
    "success": true,
    "errorCode": "",
    "errorMsg": "",
    "retry": true,
    "result": {}
}
```

返回参数：

| 字段      | 说明       | 备注 |
| --------- | ---------- | ---- |
| success   | 是否成功   |      |
| errorCode | 返回状态   |      |
| errorMsg  | 状态信息   |      |
| retry     |            |      |
| result    | 业务返回值 |      |

## 公共信息

### 10001 获取服务器时间

> 通过该接口可以获取服务器时间。

请求：

```json
{
    "domain": "cdel",
    "resourcePath": "+/server/time"
}
```

返回：

```json
{
    "success": true,
    "retry": true,
    "result": 1625194709638
}
```

result参数：

| 字段 | 说明 | 备注 |
| ---- | ---- | --- |
| 数字串 | 13位字符串时间戳 |     |

### 10002 上传用户操作记录（老接口）

> 通过该接口可以上次用户的操作记录。

请求地址：

```json
POST http://data.cdeledu.com/SaveFileData
```

请求参数：

| 字段       | 说明                                          | 备注 |
| ---------- | --------------------------------------------- | ---- |
| userID     | 用户ID                                        | 必选 |
| siteID     | 数据采集站点ID                                | 必选 |
| deviceId   | 设备序列号                                    | 必选 |
| file       | data.log文本内容                              | 必选 |
| submitTime | 提交时间                                      | 必选 |
| pkey       | Ssouid、time、cdelofflineClient加密而来的密钥 | 必选 |

返回参数：

| 字段 | 说明 | 备注 |
| ---- | ---- | --- |
| true | 成功 |     |
| false | 失败 | |

### 10101 客户端更新（老接口）

> 通过该接口可以更新客户端.

请求地址：

```http
GET http://game.chinaacc.com/CourseClientUpdate/Chinaacc/version.xml
```

返回（xml格式）：

```xml
<?xml version="1.0" encoding="utf-8"?>
<root>
  <ver>1.1.2.2</ver>
  <infos>
    <info ver="1.0.0.10">
    </info>
  </infos>
  <files>
    <file filename="CdelCourse.exe" localpath="\CdelCourse.exe" remoteurl="http://game.chinaacc.com/CourseClientUpdate/Chinaacc/CdelCourse.exe" updatecmd="" ver="1.1.2.2" filehash="CCB5DB5B071388DB93234D9A59F1B31384482679" />
    <file filename="Framework.dll" localpath="\Framework.dll" remoteurl="http://game.chinaacc.com/CourseClientUpdate/Chinaacc/Framework.dll" updatecmd="" ver="1.1.2.2" filehash="9FF4A06B56A8172644C3556DFED942E7EA2D2BBC" />
    <file filename="GalaSoft.MvvmLight.dll" localpath="\GalaSoft.MvvmLight.dll" remoteurl="http://game.chinaacc.com/CourseClientUpdate/Chinaacc/GalaSoft.MvvmLight.dll" updatecmd="" ver="1.1.0.88" filehash="" />
    <file filename="GalaSoft.MvvmLight.Extras.dll" localpath="\GalaSoft.MvvmLight.Extras.dll" remoteurl="http://game.chinaacc.com/CourseClientUpdate/Chinaacc/GalaSoft.MvvmLight.Extras.dll" updatecmd="" ver="1.1.0.88" filehash="" />
  </files>
</root>
```

返回参数：

| 字段名     | 说明                   | 备注 |
| ---------- | ---------------------- | ---- |
| ver        | 当前版本号             |      |
| infos      | （未使用）             |      |
| infos/info | （未使用）             |      |
| files      | 版本更新涉及的文件合集 |      |
| files/file | 版本更新涉及的文件     |      |

file参数：

| 属性名    | 说明       |
| --------- | ---------- |
| filename  | 文件名     |
| localpath | 本地路径   |
| remoteurl | 下载路径   |
| updatecmd | 更新命令   |
| ver       | 文件版本   |
| filehash  | 文件哈希值 |

### 10102 获取学员跑马灯信息

> 通过该接口可以从服务器获取在客户端的底部需要显示的滚动信息。

资源地址：

```http
+/dlclassroom/getMarqueeInfoListGateway
```

请求：

```json
{
    "params": "--加密串--",
    "domain": "chinaacc",
    "resourcePath": "+/dlclassroom/getMarqueeInfoListGateway",
    "platform": "",
    "appType": "",
    "appKey": "",
    "appVersion": "",
    "publicKey": "--公钥--",
    "aesKey": "--加密串--",
    "sid": ""
}
```

params参数：

| 参数           | 说明       | 备注                                                         |
| -------------- | ---------- | ------------------------------------------------------------ |
| pkey           | md5校验码  | 使用以下字段计算得到：<br />appkey、<br />platformSource、<br />time、<br />salt串 |
| appkey         | 13位字符串 |                                                              |
| platformSource | 13位字符串 |                                                              |
| time           | 13位字符串 |                                                              |
| etime          | 13位字符串 |                                                              |

返回：

```json
{
    "success": true,
    "errorCode": "",
    "errorMsg": "",
    "retry": true,
    "result": {
        "pkey": "0e82a7059b0d05b7263473af6a836c66",
        "marqueeInfoList": [
            {
                "appKey": "",
                "appName": "",
                "courseEduID": "",
                "fontColour": "",
                "id": 1,
                "operator": 36,
                "platformId": 2,
                "pushContent": "关课提示：下载课程在考试结束后一周关闭，请抓紧时间学习！",
                "pushType": 1,
                "realName": "",
                "rowNumEnd": 0,
                "rowNumStart": 0,
                "siteId": 1,
                "siteName": ""
            }
        ],
        "code": 1,
        "msg": "成功",
        "outKey": "0e82a7059b0d05b7263473af6a836c66"
    }
}
```

result参数：

| 字段名          | 说明           | 备注 |
| --------------- | -------------- | ---- |
| pkey            |                |      |
| marqueeInfoList | 跑马灯信息列表 |      |
| code            | 返回状态       |      |
| msg             | 状态信息       |      |
| outKey          |                |      |

marqueeInfoList参数：

| 属性名      | 说明               | 备注 |
| ----------- | ------------------ | ---- |
| appKey      | （暂未使用）       |      |
| appName     | （暂未使用）       |      |
| courseEduID | （暂未使用）       |      |
| fontColour  | 使用的字体颜色     |      |
| id          | （暂未使用）       |      |
| operator    | （暂未使用）       |      |
| platformId  | （暂未使用）       |      |
| pushContent | 推送的具体信息内容 |      |
| pushType    | （暂未使用）       |      |
| realName    | （暂未使用）       |      |
| rowNumEnd   | （暂未使用）       |      |
| rowNumStart | （暂未使用）       |      |
| siteId      | （暂未使用）       |      |
| siteName    | （暂未使用）       |      |

## 客户端认证

### 10202 账户冻结检测

> 通过该接口可以检测当前账户是否被冻结。

资源地址：

```http
+/appLogin/ucChkUserLogin
```

请求：

```json
{
    "params": "--加密串--",
    "domain": "chinaacc",
    "resourcePath": "+/appLogin/ucChkUserLogin",
    "platform": "",
    "appType": "",
    "appKey": "",
    "appVersion": "",
    "publicKey": "--公钥--",
    "aesKey": "--加密串--",
    "sid": ""
}
```

params参数：

| 字段名  | 说明             | 备注                                                         |
| ------- | ---------------- | ------------------------------------------------------------ |
| ltime   | token的令牌时间  |                                                              |
| pkey    | md5校验码        | 通过以下字段计算得到：<br />userID、<br />platformSource、<br />version、<br />time、<br />固定字符串"fJ3UjIFyTu"（用于加盐）、<br />token值 |
| time    | 13位字符串时间戳 |                                                              |
| etime   | 13位字符串时间戳 |                                                              |
| userID  | SsoUid           |                                                              |
| version | 程序版本号       |                                                              |

返回：

```json
{
    "success": true,
    "retry": true,
    "result": {
        "msg": "not auth",
        "code": "-1"
    }
}
```

result参数：

| 字段名 | 说明       | 备注                        |
| ------ | ---------- | --------------------------- |
| code   | 状态指示   | “1”：表示账户异常，已被冻结 |
| msg    | 返回的信息 |                             |

### 10203 账户登录

> 通过该接口可以登录客户端账号。

资源地址：

```http
+/appLogin/downUserLogin
```

请求：

```json
{
    "params": "--加密串--",
    "domain": "chinaacc",
    "resourcePath": "+/appLogin/downUserLogin",
    "platform": "10",
    "appType": "winpc",
    "appKey": "a5d25067-1f78-4a93-8060-6c6cb1492ef8",
    "appVersion": "1.3.0.0",
    "publicKey": "--公钥--",
    "aesKey": "--加密串--",
    "sid": "",
    "af": "10",
    "fs": "300",
    "ve": "1.3.0.0",
    "lt": "1625466657527",
    "ap": "a5d25067-1f78-4a93-8060-6c6cb1492ef8"
}
```

参数：(通用参数之外的参数)

| 参数 | 说明                                                         | 备注 |
| ---- | ------------------------------------------------------------ | ---- |
| af   | 域名                                                         |      |
| fs   | 目前分为两级第一级第一位标识，第23位作为二级标识<br />目前暂定1-安卓 2-ios 3-pc 4-m站 5-小程序 6-未知 7-m和pc（注意后续可能出现非数字）<br />二级标识2、3位，目前暂定01-手机 02-pad  03-TV 00-无（注意后续可能出现非数字） |      |
| ve   | 同appVersion                                                 |      |
| lt   | token的令牌时间                                              |      |
| ap   | 同appKey                                                     |      |

params参数：

| 参数             | 说明                    | 备注                                           |
| ---------------- | ----------------------- | ---------------------------------------------- |
| username         | 登录账号                |                                                |
| passwd           | 登录密码                |                                                |
| memberlevel      | （暂未明确）            | 固定字符串“pc”                                 |
| memberkey        | （暂未明确）            | 固定字符串"84772CDB"                           |
| mid              | 设备加密密钥（机器key） | 数据库表DBKey中的MainKey，或注册表中的硬件信息 |
| mname            | 本地计算机主机名        |                                                |
| muname           |                         | 空字符串                                       |
| appname          | 当前登录的程序名        | 用于区分“会计课堂”、“建筑课堂”、“医学课堂”等   |
| isNeedReplaceMid |                         | 空字符串                                       |
| platformSource   | 来源平台                | 固定字符串"member_app_passwd"                  |
| domain           | 域名                    |                                                |
| time             | 13位字符串时间戳        |                                                |

返回：

```json
{
    "success": true,
    "retry": true,
    "result": {
        "msg": "操作成功",
        "registerFlag": false,
        "code": "0",
        "nickName": "dashu",
        "ssouid": 33049216,
        "sex": "女",
        "notify": "如果您已购课，请注销后重新登录或重新打开应用",
        "sid": "8ed6772d-d429-461f-b775-71fae8d3a9b6",
        "isBindWechat": "0",
        "mobilePhone": "16603268602",
        "schoolID": "",
        "protalUserName": "zhaobin1990",
        "mlist": [],
        "fullname": "赵彬",
        "iconUrl": "http://member.chinaacc.com/homes/resources/images/home/avatar/6.jpg",
        "bindCode": "0",
        "email": "1281727579@qq.com",
        "username": "zhaobin1990"
    }
}
```

result参数：

| 字段名         | 说明               | 备注                                                         |
| -------------- | ------------------ | ------------------------------------------------------------ |
| msg            | 返回的指示信息     |                                                              |
| registerFlag   |                    |                                                              |
| code           | 登录操作状态指示   | “0”：用户名密码正确<br />“-4”：学员代码或密码错误<br />“-5”：学员代码或密码错误<br />“-12”：在太多电脑上登录了该帐号<br />“-18”：该设备已经被注销，不能再次使用<br />其它代码：登录失败<br /> |
| nickName       | 昵称               |                                                              |
| ssouid         | SsoUid             |                                                              |
| sex            | 性别               |                                                              |
| notify         | 通知消息           |                                                              |
| sid            | 当前用户唯一识别码 |                                                              |
| isBindWechat   | 是否绑定微信       |                                                              |
| mobilePhone    | 已绑定手机号       |                                                              |
| schoolID       |                    |                                                              |
| protalUserName | 用户名             |                                                              |
| mlist          | 已绑定设备列表     |                                                              |
| fullname       | 全名               |                                                              |
| iconUrl        | 头像               |                                                              |
| bindCode       |                    |                                                              |
| email          | 邮箱               |                                                              |
| username       | 用户名             |                                                              |

### 10204 上传登录记录（老接口）

> 通过该接口可以记录用户在客户端上的登录操作

请求地址：

```http
POST http://mportal.chinaacc.com/video/header/Visitor
```

请求参数：

| 参数      | 说明   | 备注 |
| --------- | ------ | ---- |
| userName  | 登录账户名 |      |
| ssoUid    | SsoUid |      |
| osVersion | 操作系统版本 |      |
| ssid      | 设备加密密钥（机器key） | 数据库表DBKey中的MainKey，或注册表中的硬件信息 |

返回：

```none
Y
```

### 10205 获取Token访问令牌

> 通过该接口可以获取token访问令牌。

资源地址：

```http
+/auth/token/getTokenGateway
```

请求：

```json
{
    "params": "--加密串--",
    "domain": "chinaacc",
    "resourcePath": "+/auth/token/getTokenGateway",
    "platform": "",
    "appType": "",
    "appKey": "",
    "appVersion": "",
    "publicKey": "--公钥--",
    "aesKey": "--加密串--",
    "sid": ""
}
```

params参数：

| 参数          | 说明             | 备注                                                         |
| ------------- | ---------------- | ------------------------------------------------------------ |
| appkey        | 程序key          | 区分程序类别（会计课堂、医学网、建设网……）                   |
| pkey          | md5校验码        | 使用下列字段计算得到：<br />platformSource、<br />version、<br />time、<br />salt盐、<br />appkey |
| platformSoure | 平台标识         | 格式yyyy-mm-dd+hh:mm:ss                                      |
| time          | 13位字符串时间戳 |                                                              |
| version       | （暂未明确）     | 固定字符串"2.0.0.0"                                          |

返回：

```json
{
    "success": true,
    "retry": true,
    "result": {
        "code": 1,
        "paramValue": "aU5c1fqNF-DB-Yzgv-NHhYRh2tBBgdRWDLJQxWl9h411clEuYrgggH9CH7DS37rMecawKLMc7JdTgp4R9kV57LA2hahO0QQ8"
    }
 }
```

result参数：

| 字段名     | 说明                  | 备注                             |
| ---------- | --------------------- | -------------------------------- |
| code       | 状态指示              | ”0“：获取失败<br />”1“：获取成功 |
| paramValue | 包含token信息的加密串 | 使用电子密码本 (ECB) 模式解密    |

paramValue参数：

| 属性名   | 说明         |
| -------- | ------------ |
| token    | 访问令牌的值 |
| longtime | 令牌时间     |
| timeout  | 有效期       |

### 10206 获取绑定设备列表

> 获取当前账户已经绑定的设备列表。

资源地址：

```http
+/appLogin/bindEquipment
```

请求：

```json
{
    "params": "--加密串--",
    "domain": "chinaacc",
    "resourcePath": "+/appLogin/bindEquipment",
    "platform": "",
    "appType": "",
    "appKey": "",
    "appVersion": "",
    "publicKey": "--公钥--",
    "aesKey": "--加密串--",
    "sid": ""
}
```

params参数：

| 字段        | 说明             | 备注                       |
| ----------- | ---------------- | -------------------------- |
| username    | 用户名           |                            |
| appname     | 程序名           | 固定字符串"AccDownClass10" |
| memberlevel | （暂未明确）     | 固定字符串"pc"             |
| memberkey   | （暂未明确）     | 固定字符串"84772CDB"       |
| time        | 13位字符串时间戳 |                            |

返回：

```json
{
    "success": true,
    "retry": true,
    "result": {
        "msg": "success",
        "code": "1",
        "ssouid": "33049216",
        "mlist": [
            {
                "mid": "438C2CCF94E5D1665CA9699869C579D4",
                "mname": "dell-PC211"
            },
            {
                "mid": "B57F8DFA4B22E1552DD8AD81087AFAA4",
                "mname": "dell-PC311"
            }
        ]
    }
}
```

result参数：

| 字段   | 说明             | 备注 |
| ------ | ---------------- | ---- |
| msg    | 状态信息         |      |
| code   | 返回状态         |      |
| ssouid | 用户id           |      |
| mlist  | 已绑定的设备列表 |      |

mlist参数：

| 字段  | 说明   | 备注 |
| ----- | ------ | ---- |
| mid   | 机器id |      |
| mname | 机器名 |      |

### 10207 发送验证码

> 发送验证码，可用于注册、绑定、vip签约等操作的身份效验。

资源地址：

```http
+/appLogin/sendMessage
```

请求：

```json
{
    "params": "--加密串--",
    "domain": "chinaacc",
    "resourcePath": "+/appLogin/sendMessage",
    "platform": "",
    "appType": "",
    "appKey": "",
    "appVersion": "",
    "publicKey": "--公钥--",
    "aesKey": "--加密串--",
    "sid": ""
}
```

params参数：

| 字段        | 说明                   | 备注                                                         |
| ----------- | ---------------------- | ------------------------------------------------------------ |
| mobilePhone | 发送验证码的目标手机号 |                                                              |
| smstype     | 验证类别               | 0：不校验是否注册<br />1：校验是否注册<br />2：校验手机号是否已被绑定<br />3：vip签约获取验证码 |
| time        | 13位字符串时间戳       |                                                              |

返回：

```json
{
    "success": true,
    "retry": true,
    "result": {
        "msg": "验证码已发送",
        "code": "1",
        "codeVerify": "2fe13b3ca5ae8e501622c409c64e6136"
    }
}
```

result参数：

| 字段       | 说明     | 备注 |
| ---------- | -------- | ---- |
| msg        | 状态信息 |      |
| code       | 返回状态 |      |
| codeVerify |          |      |

### 10208 身份效验

> 使用验证码对操作者的身份进行效验。

资源地址：

```http
+/appLogin/checkAuthCode
```

请求：

```json
{
    "params": "--加密串--",
    "domain": "chinaacc",
    "resourcePath": "+/appLogin/checkAuthCode",
    "platform": "",
    "appType": "",
    "appKey": "",
    "appVersion": "",
    "publicKey": "--公钥--",
    "aesKey": "--加密串--",
    "sid": ""
}
```

params参数：

| 字段            | 说明                   | 备注 |
| --------------- | ---------------------- | ---- |
| mobilePhone     | 发送验证码的目标手机号 |      |
| messageAuthCode | 目标手机收到的验证码   |      |
| time            | 13位字符串时间戳       |      |

返回：

```json
{
    "success": true,
    "retry": true,
    "result": {
        "msg": "校验成功！",
        "code": "1"
    }
}
```

result参数：

| 字段 | 说明     | 备注 |
| ---- | -------- | ---- |
| msg  | 状态信息 |      |
| code | 返回状态 |      |

### 10209 解绑设备

> 解绑当前账户中的指定设备。

资源地址：

```http
+/appLogin/equipmentOut
```

请求：

```json
{
    "params": "--加密串--",
    "domain": "chinaacc",
    "resourcePath": "+/appLogin/equipmentOut",
    "platform": "",
    "appType": "",
    "appKey": "",
    "appVersion": "",
    "publicKey": "--公钥--",
    "aesKey": "--加密串--",
    "sid": ""
}
```

params参数：

| 字段        | 说明             | 备注                 |
| ----------- | ---------------- | -------------------- |
| userMid     | 要解绑的设备id   |                      |
| userMUname  | 要解绑的设备名   |                      |
| appname     | 程序名           |                      |
| memberlevel | （暂未明确）     | 固定字符串“pc”       |
| memberkey   | （暂未明确）     | 固定字符串"84772CDB" |
| ssouid      | SsoUid           |                      |
| username    | 用户名           |                      |
| mid         | 机器id           |                      |
| mname       | 机器名           |                      |
| time        | 13位字符串时间戳 |                      |

返回：

```json
{
    "success": true,
    "retry": true,
    "result": {
        "code": "1"
    }
}
```

result参数：

| 字段 | 说明     | 备注 |
| ---- | -------- | ---- |
| code | 返回状态 |      |

### 10210 绑定手机号

> 更换当前账户的绑定手机号。

资源地址：

```http
+/appLogin/userAccountBind
```

请求：

```json
{
    "params": "--加密串--",
    "domain": "chinaacc",
    "resourcePath": "+/appLogin/userAccountBind",
    "platform": "",
    "appType": "",
    "appKey": "",
    "appVersion": "",
    "publicKey": "--公钥--",
    "aesKey": "--加密串--",
    "sid": ""
}
```

params参数：

| 字段            | 说明             | 备注 |
| --------------- | ---------------- | ---- |
| mobilePhone     | 需要绑定的手机号 |      |
| messageAuthCode | 收到的验证码     |      |
| ssouid          | SsoUid           |      |
| time            | 13位字符串时间戳 |      |

返回：

```json
{
    "success": true,
    "retry": true,
    "result": {
        "msg": "该手机号已被其他账号绑定！",
        "code": "-4"
    }
}
```

result参数：

| 字段 | 说明     | 备注 |
| ---- | -------- | ---- |
| msg  | 状态信息 |      |
| code | 返回状态 |      |

## 我的课程

### 10301 同步学习记录

> 通过该接口可以同步客户端上的学习记录，登录成功后会调用一次。

资源地址：

```http
+/classroom/versionm/record/saveBatchMessageGateway
```

请求：

```json
{
    "params": "--加密串--",
    "domain": "chinaacc",
    "resourcePath": "+/classroom/versionm/record/saveBatchMessageGateway",
    "platform": "",
    "appType": "",
    "appKey": "",
    "appVersion": "",
    "publicKey": "--公钥--",
    "aesKey": "--加密串--",
    "sid": ""
}
```

params参数：

| 参数           | 说明             | 备注                                                         |
| -------------- | ---------------- | ------------------------------------------------------------ |
| appkey         | 程序key          | 用于区分“会计课堂”、“建筑课堂”、“医学课堂”……                 |
| guid           | 全局唯一标识符   |                                                              |
| ltime          | token的令牌时间  |                                                              |
| online         | 是否在线标识     | “1”：表示在线<br />“0”：表示离线                             |
| pkey           | md5校验码        | 通过以下字段计算得到：<br />SsoUid、<br />platformSource、<br />version、<br />time、<br />type、<br />guid、<br />固定字符串"fJ3UjIFyTu"（用于加盐）<br />token值 |
| studyVideoJson | 学习记录信息     |                                                              |
| time           | 13位字符串时间戳 |                                                              |
| etime          | 13位字符串时间戳 |                                                              |
| type           | （暂不明确）     | 固定字符串“qz”                                               |
| uid            | SsoUid           |                                                              |
| userID         | SsoUid           |                                                              |
| version        | 程序版本号       |                                                              |

studyVideoJson参数：

| 参数     | 说明         | 备注 |
| -------- | ------------ | ---- |
| videoStr | 视频信息详情 |      |

videoStr参数：

| 参数       | 说明             | 备注 |
| ---------- | ---------------- | ---- |
| cwareID    | 课件ID           |      |
| deviceID   | 设备ID           |      |
| rangeEnd   | 视频总长度结束点 |      |
| rangeStart | 视频总长度开始点 |      |
| videoID    | 视频ID           |      |
| timebase   | 课件学习详情     |      |

timebase参数：

| 参数           | 说明           | 备注 |
| -------------- | -------------- | ---- |
| CwareId        | 课件ID         |      |
| VideoID        | 视频ID         |      |
| VideoStartTime | 视频开始时间点 |      |
| VideoEndTime   | 视频结束时间点 |      |
| Speed          | 倍速           |      |
| StudyTimeEnd   | 学习结束时间   |      |
| StudyTimeStart | 学习开始时间   |      |

返回：

```json
{
    "success": true,
    "retry": true,
    "result": {
        "code": 1,
        "msg": "成功"
    }
}
```

result参数：

| 字段名 | 说明       | 备注                                     |
| ------ | ---------- | ---------------------------------------- |
| code   | 状态指示   | “1”：表示同步成功<br />“0”：表示同步失败 |
| msg    | 状态信息 |                                          |

### 10401 获取班次类别列表

> 通过该接口可以获取班次类别列表。

资源地址：

```http
+/versionm/classroom/mycware/getUserCourseListNewGateway
```

请求：

```json
{
    "params": "--加密串--",
    "domain": "chinaacc",
    "resourcePath": "+/versionm/classroom/mycware/getUserCourseListNewGateway",
    "platform": "",
    "appType": "",
    "appKey": "",
    "appVersion": "",
    "publicKey": "--公钥--",
    "aesKey": "--公钥--",
    "sid": ""
}
```

params参数：

| 参数           | 说明               | 备注                                                         |
| -------------- | ------------------ | ------------------------------------------------------------ |
| appkey         | 程序key            | 用于区分“会计课堂”、“建筑课堂”、“医学课堂”……                 |
| ltime          | token的令牌时间    |                                                              |
| pkey           | md5校验码          | 使用下列字段计算得到：<br />sid、<br />platformSource、<br />version、<br />time、<br />固定字符串"fJ3UjIFyTu"（用于加盐）、<br />token值 |
| platformSource | 来源平台           |                                                              |
| sid            | 当前用户唯一识别码 |                                                              |
| time           | 13位字符串时间戳   |                                                              |
| etime          | 13位字符串时间戳   |                                                              |
| version        | 程序版本号         |                                                              |

返回：

```json
{
    "success": true,
    "retry": true,
    "result": {
        "uid": 33049216,
        "code": "1",
        "myCourseInfo": [
            {
                "boardID": 0,
                "eduSubjectName": "中级会计实务",
                "orderNo": 123,
                "eduSubjectID": 3099,
                "courseEduName": "中级会计职称",
                "isQuestions": 1,
                "courseEduID": 6,
                "dispOrder": 380
            }
        ],
        "subectCourseRelation": [
            {
                "eduSubjectID": 3099,
                "courseID": "acc1315190"
            }
        ]
    }
}
```

result参数：

| 字段名               | 说明         |
| -------------------- | ------------ |
| uid                  | （未使用）   |
| code                 | 返回状态   |
| myCourseInfo         | 课程信息     |
| subectCourseRelation | 课程相关信息 |

myCourseInfo参数：

| 属性名         | 说明         |
| -------------- | ------------ |
| boardID        |              |
| courseEduID    |              |
| courseEduName  |              |
| dispOrder      |              |
| eduSubjectID   | 班次类别id   |
| eduSubjectName | 班次类别名称 |
| orderNo        |              |

subectCourseRelation参数：

| 属性名       | 说明       |
| ------------ | ---------- |
| courseID     |            |
| eduSubjectID | 班次类别id |

### 10402 获取班次列表

> 通过该接口可以获取班次类别列表。

资源地址：

```http
+/versionm/classroom/mycware/getUserWareBySubjectIDGateway
```

请求：

```json
{
    "params": "--加密串--",
    "domain": "chinaacc",
    "resourcePath": "+/versionm/classroom/mycware/getUserWareBySubjectIDGateway",
    "platform": "",
    "appType": "",
    "appKey": "",
    "appVersion": "",
    "publicKey": "--公钥--",
    "aesKey": "--加密串--",
    "sid": ""
}
```

需要传递以下参数：

| 参数         | 说明               | 备注                                                         |
| ------------ | ------------------ | ------------------------------------------------------------ |
| eduSubjectID | 课程类别id         |                                                              |
| ltime        | token的令牌时间    |                                                              |
| pkey         | md5校验码          | 涉及sid、eduSubjectID、platformSource、version、固定字符串"fJ3UjIFyTu"加盐、token值、time |
| sid          | 当前用户唯一识别码 |                                                              |
| time         | 13位字符串时间戳   |                                                              |
| etime        | 13位字符串时间戳   |                                                              |
| uid          | SsoUid             |                                                              |
| version      | 程序版本号         |                                                              |
| vflag        | （暂不明确）       | 可选                                                         |

返回：

```json
{
    "success": true,
    "retry": true,
    "result": {
        "userID": "33049216",
        "cwList": [
            {
                "isMobileClass": 0,
                "boardID": 0,
                "datNum": 0,
                "mobileTitle": "先学为快",
                "cwareName": "【先学为快】中级会计实务-高志谦（2021年）-特色畅学班",
                "dateEnd": "2021-09-14 00:00:00",
                "openTime": "2020-09-15 19:39:00",
                "classOrder": 575,
                "cwareClassName": "先学为快",
                "teacherName": "高志谦",
                "cYearName": "2021年",
                "rownum": 1,
                "projectID": 0,
                "cwID": "acc1315190",
                "highClass": "0",
                "specialFlag": 1,
                "cwareClassID": 572,
                "eduOrder": 123,
                "homeShowYear": 1,
                "cwareUrl": "http://elearning.chinaacc.com/xcware/video/videoList/videoList.shtm?cwareID=36834",
                "cwareImg": "https://img.cdeledu.com/CWARE/2020/1029/dde622cf74dbf8cf-0.png",
                "queNum": 0,
                "courseOpenExplain": "您所报的课程费用已交纳，课程将于新教材发布前陆续开通，敬请关注！",
                "updateTime": "2020-12-11 14:36:00",
                "wareOrder": 12475,
                "classOrder1": 575,
                "mobileCourseOpen": 1,
                "videoType": 1,
                "cwareID": 36834,
                "yearOrder": 88,
                "isFree": 0,
                "cwareTitle": "先学为快",
                "downLoad": "Y",
                "prompt": "新教材发布前",
                "useFul": 1
            }
        ],
        "imsg": "成功",
        "code": 1,
        "edusubjectID": "3099",
        "msg": "成功",
        "videoList": [
            {
                "thisProgress": 0,
                "cwid": "acc1288091"
            }
        ]
    }
}
```

result参数：

| 字段名       | 说明                       |
| ------------ | -------------------------- |
| code         | 状态指示（暂未使用）       |
| cwList       | 班次列表信息               |
| edusubjectID | 班次类别id（暂未使用）     |
| imsg         | 消息返回（暂未使用）       |
| msg          | 消息返回（暂未使用）       |
| userID       | （暂未使用）               |
| videoList    | 班次的视频信息（暂未使用） |

cwList字段包含如下字段：

| 属性名           | 说明                                   |
| ---------------- | -------------------------------------- |
| boardID          | 答疑板id                               |
| classOrder       | 班次排序                               |
| classOrder1      |                                        |
| cwareClassID     | 课件班次id                             |
| cwareClassName   | 课件班次名称                           |
| cwareID          | 课件id（例如：37076）                  |
| cwareImg         | 课件图片                               |
| cwareName        | 课件名称                               |
| cwareTitle       | 课件标题                               |
| cwareUrl         | 课件地址                               |
| cwID             | 课件id（例如：acc121541b）             |
| cYearName        | 课程年份                               |
| dateEnd          | 关课时间                               |
| datNum           |                                        |
| downLoad         | 是否可下载                             |
| eduOrder         |                                        |
| highClass        |                                        |
| homeShowYear     |                                        |
| isFree           |                                        |
| isMobileClass    | 是否移动端课程（移动端功能已移除）     |
| mobileCourseOpen | 是否开通移动端课程（移动端功能已移除） |
| mobileTitle      | 移动端显示标题（移动端功能已移除）     |
| openTime         | 开通时间                               |
| projectID        |                                        |
| prompt           | 课件提示                               |
| queNum           |                                        |
| rownum           |                                        |
| specialFlag      |                                        |
| teacherName      | 老师名称                               |
| updateTime       | 更新时间                               |
| useFul           |                                        |
| videoType        | 视频类型                               |
| wareOrder        | 课件排序                               |
| yearOrder        | 年份排序                               |

videoList字段包含如下字段：

| 属性名       | 说明     |
| ------------ | -------- |
| cwid         | 课件id   |
| thisProgress | 视频进度 |

### 10403 获取班次内的章节、讲次信息

> 通过该接口可以获取班次内的章节和讲次的具体信息。

资源地址：

```http
+/mapi/versionm/classroom/course/getCourseDetailGw
```

请求：

```json
{
    "params": "--加密串--",
    "domain": "chinaacc",
    "resourcePath": "+/mapi/versionm/classroom/course/getCourseDetailGw",
    "platform": "",
    "appType": "",
    "appKey": "",
    "appVersion": "",
    "publicKey": "--公钥--",
    "aesKey": "--加密串--",
    "sid": ""
}
```

params参数：

| 参数            | 说明             | 备注                                                         |
| --------------- | ---------------- | ------------------------------------------------------------ |
| cwID            | 课件id           |                                                              |
| freeOpenVersion | 课件id，同cwID   |                                                              |
| getType         | （暂不明确）     | 固定字符串”2“                                                |
| innerCwareID    | （暂不明确）     | 可选                                                         |
| ltime           | token的令牌时间  |                                                              |
| pkey            | md5校验码        | 使用下列字段计算得到：<br />getType、<br />username、<br />classid、<br />platformSource、<br />version、<br />time、<br />token值、<br />固定字符串"fJ3UjIFyTu"（用于加盐） |
| platformSource  | 平台类别         | 固定字符串”10“                                               |
| time            | 13位字符串时间戳 |                                                              |
| etime           | 13位字符串时间戳 |                                                              |
| username        | 登录账户名       |                                                              |
| version         | 程序版本号       |                                                              |
| cdn             | （暂不明确）     | 固定字符串”1“                                                |
| videoType       | 视频类型         | 固定字符串”1“                                                |

返回：

```json
{
    "success": true,
    "retry": true,
    "result": {
        "ret": "--加密串--"
        "code": 1
    }
}
```

result参数：

| 字段名 | 说明               | 备注                                     |
| ------ | ------------------ | ---------------------------------------- |
| code   | 状态指示           | ”1“：表示获取成功<br />”0“：表示获取失败 |
| ret    | 返回的班次明细信息 | 使用电子密码本 (ECB) 模式解密            |

ret参数解密后：

```json
{
    "chapterlist": [
        {
            "order": "100",
            "outchapterID": "0",
            "chaptertname": "第一部分　前　言",
            "chapterid": "127026"
        }
    ],
    "updateTime": "2021-04-26 14:24:14",
    "courseware": [
        {
            "videoname": "第01讲　其他相关知识介绍",
            "pointname": "",
            "demotype": "0",
            "pointid": "",
            "videoOrder": "21",
            "videozipurl": "http://downcdn.chnedu.com/downmf/hd2021/zhukuai/lijichu/kj_lgf/phone/acc131541a0301_v.zip",
            "videoHDzipurl": "http://downcdn.chnedu.com/downmf/hd2021/zhukuai/lijichu/kj_lgf/Ipad/acc131541a0301_v.zip",
            "chapterid": "127125",
            "audiourl": "/sec.chnedu.com/PT2GJzzBYHhZsV-d7FHGtXUNt.lTCMvs018lJU.gz.Z3fWnRifBbVMyc6aPZGfaVE-H0Qqhajdw_.mp4",
            "videourl": "/sec.chnedu.com/PT2GJzzBYHhZsV-d7FHGtXUNt.lTCMvs018lJU.gz.buDuVXLiSl3cyc6aPZGfaVE-H0Qqhajdw_.mp4",
            "modTime": "",
            "title": "第01讲　其他相关知识介绍",
            "audiozipurl": "http://downcdn.chnedu.com/downmf/hd2021/zhukuai/lijichu/kj_lgf/phone/acc131541a0301_a.zip",
            "NodeID": "0301",
            "videoHDurl": "/sec.chnedu.com/PT2GJzzBYHhZsV-d7FHGtXUNt.lTCMvs018lJU.gz.buDuVXLiSl3cyc6aPZGfaVE-H0Qqhajdw_.mp4",
            "length": "01:01:41",
            "videotype": "1"
        }
    ]
}
```

chapterlist参数：

| 属性名       | 说明         |
| ------------ | ------------ |
| order        | 排序号       |
| outchapterID | （暂未使用） |
| chaptertname | 章节名称     |
| chapterid    | 章节id       |

courseware字段包含如下字段：

| 属性名        | 说明                                                         |
| ------------- | ------------------------------------------------------------ |
| videoname     | 视频名称                                                     |
| pointname     | 知识点名称                                                   |
| demotype      | 试听                                                         |
| pointid       | 知识点id                                                     |
| videoOrder    | 视频排序号                                                   |
| videozipurl   | 视频zip地址（标清）                                          |
| videoHDzipurl | 视频zip地址（高清）                                          |
| chapterid     | 章节id                                                       |
| audiourl      | 音频地址                                                     |
| videourl      | 视频地址                                                     |
| modTime       | 视频更新时间                                                 |
| title         | 显示名称                                                     |
| audiozipurl   | 音频zip地址                                                  |
| NodeID        | 视频id                                                       |
| videoHDurl    | 视频地址（高清）                                             |
| length        | 视频长度                                                     |
| videotype     | 视频类型：<br />”1“：zip<br />”2“：媒体文件<br />”3“：仅讲义 |

### 10404 产生视频密钥（老接口）

> 通过该接口可以从服务器获取视频密钥

请求地址：

```http
POST http://mportal.chinaacc.com/video/header/genkey
```

返回：

```none
FEE222F11080552AA434455F169437E3D22D120D
```

### 10405 获取视频密钥

> 通过该接口可以从服务器获取视频密钥。

资源地址：

```http
+/versionm/classroom/course/getCwareKeyGateway
```

请求：

```json
{
    "params": "--加密串--",
    "domain": "chinaacc",
    "resourcePath": "+/versionm/classroom/course/getCwareKeyGateway",
    "platform": "",
    "appType": "",
    "appKey": "",
    "appVersion": "",
    "publicKey": "--公钥--",
    "aesKey": "--加密串--",
    "sid": ""
}
```

需要传递以下参数：

| 参数           | 说明             | 备注                                                         |
| -------------- | ---------------- | ------------------------------------------------------------ |
| cwID           | 课件id           |                                                              |
| cwareKey       | （暂不明确）     | 可选                                                         |
| ltime          | token的令牌时间  |                                                              |
| pkey           | md5校验码        | 通过以下字段计算得到：<br />cwID、<br />platformSource、<br />version、<br />time、<br />token值、<br />固定字符串"fJ3UjIFyTu"（用于加盐） |
| platformSource | 来源平台         |                                                              |
| time           | 13位字符串时间戳 |                                                              |
| etime          | 13位字符串时间戳 |                                                              |
| version        | 程序版本号       |                                                              |

返回：

```json
{
    "success": true,
    "retry": true,
    "result": {
        "code": "1",
        "cwareKey": "c1d5e24e"
    }
}
```

result参数：

| 字段名   | 说明              |
| -------- | ----------------- |
| code     | 返回状态      |
| cwarekey | 视频密钥，长度为8 |

### 10406 获取视频文件头（老接口）

> 通过该接口可以从服务器获取视频密钥。

请求地址：

```http
POST http://mportal.chinaacc.com/video/header/GetHead
```

请求参数：

| 参数          | 说明                       | 备注                                                     |
| ------------- | -------------------------- | -------------------------------------------------------- |
| userName      | 登录账户名                 |                                                          |
| cwareid       | 课件id                     |                                                          |
| videoid       | 视频id                     |                                                          |
| videoType     | 视频类型                   |                                                          |
| hash          | 文件hash值                 |                                                          |
| priKey        | md5加密后的密钥key         | 通过以下字段计算得到：<br />hash、<br />生成的加密随机数 |
| time          | zip包里的视频时间戳        |                                                          |
| videoHeadhash | zip包里的md5加密后的hash值 |                                                          |

返回：

```json

```

### 10407 获取视频文件头——直接获取，不带密钥（老接口）

> 通过该接口可以从服务器获取视频密钥。

请求地址：

```http
POST http://mportal.chinaacc.com/video/header/GetHeader
```

请求参数：

| 参数          | 说明                       | 备注 |
| ------------- | -------------------------- | ---- |
| userName      | 登录账户名                 |      |
| cwareid       | 课件id                     |      |
| videoid       | 视频id                     |      |
| videoType     | 视频类型                   |      |
| hash          | 文件hash值                 |      |
| time          | zip包里的视频时间戳        |      |
| videoHeadhash | zip包里的md5加密后的hash值 |      |

返回：

```json

```

### 10408 获取视频对应知识点测试及弹出时间

> 通过该接口可以获取视频所对应的知识点以及弹出的时间。

资源地址：

```http
+/classroom/versionm/cware/getPointTestStartTimeGateway
```

请求：

```json
{
    "params": "--加密串--",
    "domain": "chinaacc",
    "resourcePath": "+/classroom/versionm/cware/getPointTestStartTimeGateway",
    "platform": "",
    "appType": "",
    "appKey": "",
    "appVersion": "",
    "publicKey": "--公钥--",
    "aesKey": "--加密串--",
    "sid": ""
}
```

需要传递以下参数：

| 参数           | 说明   | 备注 |
| -------------- | ------ | ---- |
| cwareID        | 课件id |      |
| ltime          | token的令牌时间 |      |
| pkey           | md5校验码 | 通过以下字段计算得到：<br />SsoUid、<br />platformSource、<br />version、<br />time、<br />type、<br />guid、<br />固定字符串"fJ3UjIFyTu"（用于加盐）<br />token值 |
| platformSource | 平台类别 | 固定字符串”10“ |
| time           | 13位字符串时间戳 |    |
| etime          | 13位字符串时间戳 |    |
| version        | 程序版本号 |      |
| videoID        | 视频id |      |
| isMobileClass  | 是否为移动课堂（移动课堂功能已移除） | “1”：移动班<br />“2”：下载课堂 |

返回：

```json
{
    "success": true,
    "retry": true,
    "result": {
        "code": 1,
        "pointTestTimeList": [
            {
                "backTime": 218,
                "pointName": "职工薪酬的内容",
                "testID": 29118527,
                "pointTestStartTime": 756,
                "rowNum": 1,
                "pointOpenType": "t",
                "cwareConfig": "10000000000000110100010000000000"
            }
        ]
    }
}
```

result参数：

| 字段名            | 说明                   | 备注 |
| ----------------- | ---------------------- | ---- |
| code              | 返回状态           |      |
| pointTestTimeList | 知识点测试以及弹出时间 |      |

pointTestTimeList参数：

| 属性名             | 说明           | 备注     |
| ------------------ | -------------- | -------- |
| backTime           |                |          |
| pointName          | 知识点名称     |          |
| testID             | 知识点id       |          |
| pointTestStartTime | 知识点弹出时间 | 单位：秒 |
| rowNum             |                |          |
| pointOpenType      | 知识点类型     |          |
| cwareConfig        |                |          |

### 10409 获取知识点测试的题目信息

> 通过该接口可以获取知识点测试的题目信息。

资源地址：

```http
+/classroom/versionm/record/getQuestionByPointTestIDGateway
```

请求：

```json
{
    "params": "--加密串--",
    "domain": "chinaacc",
    "resourcePath": "+/classroom/versionm/record/getQuestionByPointTestIDGateway",
    "platform": "",
    "appType": "",
    "appKey": "",
    "appVersion": "",
    "publicKey": "--公钥--",
    "aesKey": "--加密串--",
    "sid": ""
}
```

params参数：

| 参数           | 说明   | 备注 |
| -------------- | ------ | ---- |
| ltime          | token的令牌时间 |      |
| pkey           | md5校验码 | 通过以下字段计算得到：<br />testID、<br />pointOpenType、<br />platformSource、<br />version、<br />time<br />固定字符串"fJ3UjIFyTu"（用于加盐）<br />token值 |
| platformSource | 平台类别 | 固定字符串”10“ |
| pointOpenType  | 知识点类型 |      |
| testID         | 知识点id |      |
| time           | 13位字符串时间戳 |    |
| etime          | 13位字符串时间戳 |    |
| version        | 程序版本号 |      |

返回：

```json
{
    "success": true,
    "retry": true,
    "result": {
        "isNewQZ": 1,
        "code": 1,
        "questionsList": [
            {
                "quesTypeID": 1,
                "pointTestID": "29118527",
                "questionID": 12859846,
                "score": 1.5,
                "answerTotal": 1,
                "analysis": "选项A，企业根据经营业绩或职工贡献等情况提取的奖金，属于奖金计划，应当比照短期利润分享计划进行处理。选项B，属于离职后福利。",
                "splitScore": 0,
                "pointID": 215814,
                "content": "下列属于其他长期职工福利的是（　）。",
                "parentID": 0,
                "quesViewType": 1,
                "answerRightTotal": 1,
                "rightAnswer": "C",
                "optionList": [
                    {
                        "questionID": 12859846,
                        "quesOption": "企业根据经营业绩或职工贡献等情况提取的奖金",
                        "quesValue": "A",
                        "sequence": 0
                    }
                ],
                "quesType": 1
            }
        ]
    }
}
```

result参数：

| 字段名       | 说明     | 备注 |
| ------------ | -------- | ---- |
| code         | 状态指示 |      |
| isNewQZ      |          |      |
| questionList |          |      |

questionList字段的列表项包含如下字段：

| 属性名           | 说明               | 备注                                                         |
| ---------------- | ------------------ | ------------------------------------------------------------ |
| analysis         | （暂未使用）       | 可选                                                         |
| answerRightTotal | 正确答案个数       | 单选1，多选有多个                                            |
| answerTotal      |                    |                                                              |
| content          | 题目内容           |                                                              |
| optionList       | 选项列表           |                                                              |
| parentID         |                    |                                                              |
| pointID          |                    |                                                              |
| pointTestID      | 知识点id           |                                                              |
| questionID       | 题目id             |                                                              |
| quesType         | 题目类型           | “1”：单项选择题<br />“2”：多项选择题<br />“3”：判断题<br />“4“：其它题型 |
| quesTypeID       |                    |                                                              |
| quesViewType     |                    |                                                              |
| rightAnswer      | 正确答案的选项标题 |                                                              |
| score            | 题目分数           |                                                              |
| splitScore       |                    |                                                              |

optionList字段的列表项包含如下字段：

| 属性名     | 说明     | 备注 |
| ---------- | -------- | ---- |
| quesOption | 选项内容 |      |
| questionID | 题目id   |      |
| quesValue  | 选项标题 |      |
| sequence   | 选项序号 |      |

### 10410 上传知识点测试的做题结果

> 通过该接口可以上传知识点测试题目的做题结果。

资源地址：

```http
+/classroom/versionm/record/savePointTestQzResultGateway
```

请求：

```json
{
    "params": "--加密串--",
    "domain": "chinaacc",
    "resourcePath": "+/classroom/versionm/record/savePointTestQzResultGateway",
    "platform": "",
    "appType": "",
    "appKey": "",
    "appVersion": "",
    "publicKey": "--公钥--",
    "aesKey": "--加密串--",
    "sid": ""
}
```

params参数：

| 参数          | 说明             | 备注                                                         |
| ------------- | ---------------- | ------------------------------------------------------------ |
| cwID          | 课程id           |                                                              |
| ltime         | token的令牌时间  |                                                              |
| pkey          | md5校验码        | 通过以下字段计算得到：<br />SsoUid、<br />testID、<br />platformSource、<br />version、<br />time、<br />固定字符串"fJ3UjIFyTu"（用于加盐）<br />token值 |
| platformSource | 来源平台       |                                                              |
| pointOpenType | 知识点类型       |                                                              |
| questionsInfo | 题目结果信息     |                                                              |
| siteCourseID  |                  |                                                              |
| testID        | 知识点id         |                                                              |
| time           | 13位字符串时间戳 |    |
| etime          | 13位字符串时间戳 |    |
| userID        | SsoUid           |                                                              |
| version       | 程序版本号       |                                                              |

questionsInfo参数：

| 属性名    | 说明     | 备注 |
| --------- | -------- | ---- |
| questions | 题目合集 |      |

questions参数：

| 属性名     | 说明         | 备注 |
| ---------- | ------------ | ---- |
| questionID | 题目id       |      |
| userAnswer | 用户答题结果 |      |

返回：

```json
{
    "success": true,
    "retry": true,
    "result": {
        "isNewQZ": 1,
        "code": 1,
        "msg": "保存成功"
    }
}
```

result参数：

| 字段名  | 说明         | 备注 |
| ------- | ------------ | ---- |
| isNewQZ | （暂未使用） |      |
| code    | 返回状态 |      |
| msg     | 返回的消息   |      |

### 10411 同步视频观看记录

> 通过该接口可以同步视频观看记录，保存上一次观看的位置用于下次打开。

资源地址：

```http
/versionm/classroom/course/saveNextBeginTimeGateway
```

请求：

```json
{
    "params": "--加密串--",
    "domain": "chinaacc",
    "resourcePath": "+/versionm/classroom/course/saveNextBeginTimeGateway",
    "platform": "",
    "appType": "",
    "appKey": "",
    "appVersion": "",
    "publicKey": "--公钥--",
    "aesKey": "--加密串--",
    "sid": ""
}
```

result参数：

| 参数           | 说明             | 备注                                                         |
| -------------- | ---------------- | ------------------------------------------------------------ |
| history        | 观看历史         |                                                              |
| ltime          | token的令牌时间  |                                                              |
| pkey           | md5校验码        | 通过以下字段计算得到：<br />history、<br />platformSource、<br />version、<br />time、<br />token值、<br />固定字符串"fJ3UjIFyTu"（用于加盐） |
| platformSource | 来源平台         | 固定字符串“10”                                               |
| time           | 13位字符串时间戳 |                                                              |
| version        | 程序版本号       |                                                              |

history字段的列表项包含如下字段：

| 属性名         | 说明             | 备注 |
| -------------- | ---------------- | ---- |
| cwareUrl       | 视频地址         |      |
| cwareid        | 课件id           |      |
| nextBegineTime | 最后观看到的时间 |      |
| uid            | 用户id           |      |
| updateTime     | 更新时间         |      |
| videoid        | 视频id           |      |


返回：

```json
{
    "success": true,
    "retry": true,
    "result": {
        "code": 1,
        "msg": "成功"
    }
}
```

result参数：

| 字段名 | 说明         | 备注 |
| ------ | ------------ | ---- |
| code   | 返回状态 |      |
| msg    | 状态信息   |      |

## 课程讲义

### 10503 获取课程讲义下载接口

> 通过该接口可以获取课程讲义的下载地址。

资源地址：

```http
+/mapi/qzdownload/phone/cware/getSmallListKcjyWordUrlGw
```

请求：

```json
{
    "params": "--加密串--",
    "domain": "chinaacc",
    "resourcePath": "+/mapi/qzdownload/phone/cware/getSmallListKcjyWordUrlGw",
    "platform": "",
    "appType": "",
    "appKey": "",
    "appVersion": "",
    "publicKey": "--公钥--",
    "aesKey": "--加密串--",
    "sid": ""
}
```

params参数：

| 字段           | 说明                                                         | 备注           |
| -------------- | ------------------------------------------------------------ | -------------- |
| cwID           | 讲义ID                                                       | 必选           |
| ltime          | 访问令牌时间                                                 | 必选           |
| pkey           | 通过SsoUid、cwID、PlatformSource、version、time、token、salt加密而来的密钥 | 必选           |
| platformSource | 来源平台                                                     | 固定字符串“10” |
| time           | 13位字符串时间戳                                             |                |
| etime          | 13位字符串时间戳                                             |                |
| userID         | 用户ID                                                       | 必选           |
| version        | 客户端版本号                                                 | 必选           |

返回：

```json
{
    "success": true,
    "retry": true,
    "result": {
        "cwareClassList": [
            {
                "smallListName": "专题一　会计基础知识理论",
                "cwID": "acc1315190  ",
                "jiangIiFile": "/zhongji/2021/yuxi/kjsw_gzq/kjsw_ljcyx_gzq_jy0101.doc",
                "smallOrder": 100,
                "smallListID": 124751
            }
        ],
        "code": 1,
        "msg": "成功"
    }
}
```

result参数：

| 字段           | 说明                                    |
| -------------- | --------------------------------------- |
| code           | 结果代码，0为失败，1为成功              |
| msg            | 结果消息，未用到                        |
| cwareClassList | StudentWareKcjyDown（课程讲义下载）集合 |

StudentWareKcjyDown参数：

| 字段          | 说明         |
| ------------- | ------------ |
| samllListName | 名称         |
| cwID          | 讲义ID       |
| jiangIiFile   | 讲义下载地址 |
| smallOrder    | 排序         |
| smallListId   |              |

## 我的题库

### 10601 根据科目ID获取考试中心列表

> 通过该接口可以获取考试中心列表。

资源地址：

```http
+/classroom/versionm/center/getPaperCentersBySubjectIDGateway
```

请求：

```json
{
    "params": "--加密串--",
    "domain": "chinaacc",
    "resourcePath": "+/classroom/versionm/center/getPaperCentersBySubjectIDGateway",
    "platform": "",
    "appType": "",
    "appKey": "",
    "appVersion": "",
    "publicKey": "--公钥--",
    "aesKey": "--加密串--",
    "sid": ""
}
```

params参数：

| 字段         | 说明                                                         | 备注 |
| ------------ | ------------------------------------------------------------ | ---- |
| eduSubjectID | 科目ID                                                       | 必选 |
| ltime        | 访问令牌时间                                                 | 必选 |
| pkey         | 通过eduSubjectID、SsoUid、PlatformSource、Version、Salt、token加密而来的密钥 | 必选 |
| platformSource         | 来源平台                          | 必选 |
| time         | 13位字符串时间戳                          | 必选 |
| etime         | 13位字符串时间戳                          | 必选 |
| userID       | 用户ID                                                       | 必选 |
| version       | 程序版本                                                       | 必选 |

返回：

```json
{
    "success": true,
    "retry": true,
    "result": {
        "isNewQZ": 1,
        "code": 1,
        "centerList": [
            {
                "siteCourseID": 16034,
                "centerParam": "",
                "sequence": 1,
                "description": "特色畅学先学为快（零基础）",
                "centerName": "2021年先学为快练习",
                "provideUser": "",
                "openStatus": 1,
                "centerID": 28995085,
                "CenterYear": 2021,
                "centerType": 3
            }
        ]
    }
}
```

result参数：

| 字段       | 说明                               |
| ---------- | ---------------------------------- |
| isNewQZ    | 未使用                             |
| code       | 结果代码，1为成功                  |
| centerList | StudentPaperCenter（考试中心）集合 |

StudentPaperCenter包含如下字段：

| 字段         | 说明         |
| ------------ | ------------ |
| centerParam  |              |
| siteCourseID |              |
| sequence     |              |
| description  | 描述         |
| centerName   | 名称         |
| providerUser | 未用到       |
| openStatus   |              |
| centerYear   | 考试中心年份 |
| centerID     | 考试中心ID   |
| centerType   | 考试中心类型 |

### 10602 根据考试中心ID获取试卷列表

> 通过该接口可以获取试卷列表

资源地址为：

```http
+/classroom/versionm/center/getNewCenterPapersGateway
```

请求：

```json
{
    "params": "--加密串--",
    "domain": "chinaacc",
    "resourcePath": "+/classroom/versionm/center/getNewCenterPapersGateway",
    "platform": "",
    "appType": "",
    "appKey": "",
    "appVersion": "",
    "publicKey": "--公钥--",
    "aesKey": "--加密串--",
    "sid": ""
}
```

params参数：

| 字段           | 说明                                                         | 备注             |
| -------------- | ------------------------------------------------------------ | ---------------- |
| centerID       | 考试中心ID                                                   | 必选             |
| ltime          | Token时间戳                                                  | 必选             |
| pkey           | centerId、platformSource、version、time、salt、token加密而来的密钥 | 必选             |
| platformSource | 来源平台                                                             | 必选，默认值为10 |
| time           | 13位字符串时间戳                          | 必选             |
| etime           | 13位字符串时间戳                          | 必选             |
| version        | 客户端版本号                                                 | 必选             |
| fordown        | 默认“1”                                                 | 必选             |

返回：

```json
{
    "success": true,
    "retry": true,
    "result": {
        "isNewQZ": 1,
        "centerPaperList": [
            {
                "createTime": "1604541345000",
                "totalScore": 2021,
                "paperName": "第一部分 会计基础知识理论",
                "courseID": 0,
                "paperID": 28995053,
                "status": 1,
                "isContest": 0,
                "paperOpenStatus": 0,
                "paperCatID": 14,
                "contestTimes": 0,
                "paperYear": 2021,
                "creator": "bianjunying",
                "paperViewID": 28995054,
                "chapter": "",
                "contestTimeLimit": 65,
                "isFree": 1,
                "chapterID": 28968808,
                "sequence": 1,
                "suitNum": 0,
                "paperViewName": "第一部分 会计基础知识理论"
            }
        ],
        "code": 1
    }
}
```

result参数：

| 字段            | 说明                         |
| --------------- | ---------------------------- |
| isNewQZ         | 未使用                       |
| code            | 结果代码，0为失败，1为成功   |
| centerPaperList | StudentPaper（试卷信息）集合 |

StudentPaper（试卷信息）参数：

| 字段             | 说明                       |
| ---------------- | -------------------------- |
| ceateTime        | 创建时间戳                 |
| totalScore       | 试卷总分                   |
| paperName        | 试卷名称                   |
| courseID         | 课程ID                     |
| paperID          | 试卷ID                     |
| status           | 0否，1是                   |
| isContest        | 是否竞赛，0否，1是，未用到 |
| paperOpenStatus  | 未用到                     |
| paperCatID       |                            |
| contestTimes     | 试卷限制提交次数，未用到   |
| paperYear        | 试卷年份                   |
| creator          | 创建人                     |
| paperVIewID      | 未用到                     |
| chapter          | 未用到                     |
| contestTimeLimit | 做题建议时间值，未用到     |
| isFree           | 未用到                     |
| chapterID        | 章节ID                     |
| sequence         | 未用到                     |
| suitNum          |                            |
| paperViewName    | 未用到                     |

### 10603 根据考试中心ID获取试卷大类

> 通过该接口可以获取试卷大类。
资源地址：

```http
+/classroom/versionm/center/getCenterPaperPartsGateway
```

请求：

```json
{
    "params": "--加密串--",
    "domain": "chinaacc",
    "resourcePath": "+/classroom/versionm/center/getCenterPaperPartsGateway",
    "platform": "",
    "appType": "",
    "appKey": "",
    "appVersion": "",
    "publicKey": "--公钥--",
    "aesKey": "--加密串--",
    "sid": ""
}
```

需要传递以下参数：

| 字段     | 说明                                                         | 备注 |
| -------- | ------------------------------------------------------------ | ---- |
| centerID | 考试中心ID                                                   | 必选 |
| ltime    | 令牌时间戳                                                   | 必选 |
| pkey     | centerId、platformSource、version、time、salt、token加密而来的密钥 | 必选 |
| time     | 13位字符串时间戳                          | 必选 |
| etime     | 13位字符串时间戳                          | 必选 |
| version  | 客户端版本号                                                 | 必选 |
| fordown   | 默认“1”                                       | 必选    |

返回：

```json
{
    "success": true,
    "retry": true,
    "result": {
        "isNewQZ": 1,
        "paperPartList": [
            {
                "quesTypeID": "",
                "createTime": 1617000727000,
                "quesViewType": 29252581,
                "paperID": {},
                "sequence": {},
                "partName": "判断题",
                "partID": 29252581,
                "randomNum": "",
                "creator": {}
            }
        ],
        "code": 1
    }
}
```

返回数据包含如下字段：

| 字段          | 说明                             |
| ------------- | -------------------------------- |
| isNewQZ       |                                  |
| paperPartList | StudentPaperPart（试卷大类）集合 |
| code          | 结果代码，0为失败，1为成功       |

StudentPaperPart（试卷大类）包含如下字段：

| 字段         | 说明         |
| ------------ | ------------ |
| quesTypeID   | 未用到       |
| createTime   | 创建时间     |
| quesViewType |              |
| paperID      | 试卷ID       |
| sequence     |              |
| partName     | 试卷大类名称 |
| partID       | 试卷大类ID   |
| randomNum    |              |
| creator      | 创建者       |

### 10604 根据考试中心ID获取对外试卷

> 通过该接口可以获取对外试卷。

资源地址：

```http
+/mapi/qzdownload/phone/paper/getCenterPaperViewsGateway
```

请求：

```json
{
    "params": "--加密串--",
    "domain": "chinaacc",
    "resourcePath": "+/mapi/qzdownload/phone/paper/getCenterPaperViewsGateway",
    "platform": "",
    "appType": "",
    "appKey": "",
    "appVersion": "",
    "publicKey": "--公钥--",
    "aesKey": "--加密串--",
    "sid": ""
}
```

params参数：

| 字段     | 说明                                                         | 备注 |
| -------- | ------------------------------------------------------------ | ---- |
| centerID | 考试中心ID                                                   | 必选 |
| ltime    | 令牌时间戳                                                   | 必选 |
| pkey     | centerId、platformSource、version、time、salt、token加密而来的密钥 | 必选 |
| time     | 13位字符串时间戳                          | 必选 |
| etime     | 13位字符串时间戳                          | 必选 |
| updateTime     | 更新时间                          | 必选 |
| version  | 客户端版本号                                                 | 必选 |
| paperType   | 默认“1”                     | 必选  |
| fordown   | 默认“1”                     | 必选  |

返回：

```json
{
    "success": true,
    "retry": true,
    "result": {
        "isNewQZ": 1,
        "questionTypeList": [
            {
                "paperType": 0,
                "createTime": "1604541345000",
                "status": 1,
                "paperID": 28995053,
                "paperParam": "",
                "paperOpenStatus": 0,
                "isContest": 0,
                "contestEndTime": "",
                "dayiID": 0,
                "contestTimes": 0,
                "creator": "bianjunying",
                "paperViewID": 28995054,
                "modifyStatus": 0,
                "contestTimeLimit": 65,
                "explainURL": "",
                "description": "",
                "sequence": 1,
                "paperViewCatID": 0,
                "openStatus": 0,
                "paperViewName": "第一部分 会计基础知识理论",
                "contestStartTime": ""
            }
        ],
        "code": 1,
        "msg": "成功"
    }
}
```

result参数：

| 字段             | 说明                             |
| ---------------- | -------------------------------- |
| isNewQZ          | 未用到                           |
| questimeTypeList | StudentPaperView（对外试卷）集合 |
| code             | 结果代码，0为失败，1为成功       |
| msg              | 结果信息                         |

StudentPaperView（对外试卷）包含如下字段：

| 字段             | 说明                        |
| ---------------- | --------------------------- |
| paperType        | 试卷类型                    |
| createTime       | 创建时间                    |
| status           |                             |
| paperID          | 试卷ID                      |
| paperParam       |                             |
| paperOpenStatus  | 未用到                      |
| isContest        |                             |
| contestEndTime   |                             |
| dayiID           |                             |
| contestTimes     | 做题次数，大于0则有数次限制 |
| creator          | 创建者                      |
| modifyStatus     |                             |
| paperViewID      |                             |
| contestTimeLimit |                             |
| sequence         |                             |
| description      | 描述                        |
| explainURL       |                             |
| paperViewCatID   |                             |
| openStatus       |                             |
| paperViewName    |                             |
| contestStartTime |                             |

### 10605 获取试卷下的题目

> 通过该接口可以获取试卷下的题目

资源地址：

```http
+/mapi/qzdownload/phone/question/getPaperQuestionInfosGateway
```

请求：

```json
{
    "params": "--加密串--",
    "domain": "chinaacc",
    "resourcePath": "+/mapi/qzdownload/phone/question/getPaperQuestionInfosGateway",
    "platform": "",
    "appType": "",
    "appKey": "",
    "appVersion": "",
    "publicKey": "--公钥--",
    "aesKey": "--加密串--",
    "sid": ""
}
```

params参数：

| 字段        | 说明                                                  | 备注 |
| ----------- | ----------------------------------------------------- | ---- |
| ltime       | 令牌时间戳                                            | 必选 |
| paperviewID |                                                       | 必选 |
| pkey        | paperViewID、version、time、salt、token加密而来的密钥 | 必选 |
| platformSource | 来源平台 | 固定字符串“10” |
| rowNumEnd   |                                                       | 可选 |
| rowNumStart |                                                       | 可选 |
| time     | 13位字符串时间戳                          | 必选 |
| etime     | 13位字符串时间戳                          | 必选 |
| updateTime  | 更新时间                                              | 可选 |
| version     | 客户端版本号                                          | 必选 |
| fordown   | 默认“1”                     | 必选  |

返回：

```json
{
    "success": true,
    "retry": true,
    "result": {
        "questionList": [
            {
                "quesTypeID": 1,
                "createTime": "",
                "limitMinute": 120,
                "lecture": "",
                "status": 1,
                "questionID": 20085942,
                "answer": "D",
                "score": 1.5,
                "analysis": "可理解性要求企业提供的会计信息应当清晰明了，便于投资者等财务报告使用者理解和使用。",
                "splitScore": 0,
                "creator": 0,
                "content": "企业提供的会计信息应当清晰明了，便于财务会计报告使用者理解和使用，这句话体现的会计信息质量要求是（　）。",
                "modifyStatus": 0,
                "parentID": 0,
                "quesViewType": 1,
                "rowNum": 0,
                "optNum": 0
            }
        ],
        "isNewQZ": 1,
        "code": 1,
        "msg": "成功"
    }
}
```

result参数：

| 字段         | 说明                        |
| ------------ | --------------------------- |
| questionList | StudentQuestion（题目）集合 |
| isNewQZ      | 未用到                      |
| code         | 结果代码，0为失败，1为成功  |
| msg          | 结果消息                    |

StudentQuestion（题目）包含如下字段：

| 字段         | 说明     |
| ------------ | -------- |
| quesTypeID   |          |
| createTime   | 创建时间 |
| limitMinute  |          |
| lecture      |          |
| status       |          |
| questionID   |          |
| answer       | 答案     |
| score        |          |
| analysis     | 解析     |
| splitScore   |          |
| creator      | 创建人   |
| content      | 问题内容 |
| modifyStatus |          |
| parentID     |          |
| quesViewType |          |
| rowNum       | 未用到   |
| optNum       | 未用到   |

### 10606 根据试卷ID获取的所有选项

> 通过该接口可以获取试卷下的所有选项。

资源地址：

```http
/mapi/qzdownload/phone/question/getPaperQuestionOptionsGateway
```

请求：

```json
{
    "params": "--加密串--",
    "domain": "chinaacc",
    "resourcePath": "+/mapi/qzdownload/phone/question/getPaperQuestionOptionsGateway",
    "platform": "",
    "appType": "",
    "appKey": "",
    "appVersion": "",
    "publicKey": "--公钥--",
    "aesKey": "--加密串--",
    "sid": ""
}
```

params参数：

| 字段        | 说明                                                  | 备注 |
| ----------- | ----------------------------------------------------- | ---- |
| ltime       | 令牌时间戳                                            | 必选 |
| paperViewID |                                                       | 必选 |
| pkey        | paperViewID、version、time、salt、token加密而来的密钥 | 必选 |
| platformSource | 来源平台 | 固定字符串“10” |
| rowNumEnd   |                                                       | 可选 |
| rowNumStart |                                                       | 可选 |
| time     | 13位字符串时间戳                          | 必选 |
| etime     | 13位字符串时间戳                          | 必选 |
| updateTime  | 更新时间                                              | 可选 |
| version     | 客户端版本号                                          | 必选 |
| fordown   | 默认“1”                     | 必选  |

返回：

```json
{
    "success": true,
    "retry": true,
    "result": {
        "questionOptionList": [
            {
                "quesValue": "A",
                "quesOption": "相关性",
                "sequence": 1,
                "questionID": 20085942,
                "rowNum": 1
            }
        ],
        "isNewQZ": 1,
        "code": 1,
        "msg": "成功"
    }
}
```

result参数：

| 字段               | 说明                           |
| ------------------ | ------------------------------ |
| isNewQZ            | 未用到                         |
| questionOptionList | questionOption（问题选项）集合 |
| code               | 结果代码，0为失败，1为成功     |
| msg                | 结果消息                       |

StudentQuestionOption（问题选项）包含如下字段：

| 字段       | 说明       |
| ---------- | ---------- |
| questionID | 问题ID     |
| quesOption | 选项文本值 |
| quesValue  | 选项具体值 |
| sequence   |            |
| rowNum     | 未用到     |

### 10607 保存做题记录

> 通过该接口可以保存做题的记录，请注意与同步学习记录的区别在于参数，在提交卷子的时候触发调用。

资源地址：

```http
+/classroom/versionm/record/saveBatchMessageGateway
```

请求：

```json
{
    "params": "--加密串--",
    "domain": "chinaacc",
    "resourcePath": "+/classroom/versionm/record/saveBatchMessageGateway",
    "platform": "",
    "appType": "",
    "appKey": "",
    "appVersion": "",
    "publicKey": "--公钥--",
    "aesKey": "--加密串--",
    "sid": ""
}
```

params参数：

| 字段        | 说明                                                         | 备注 |
| ----------- | ------------------------------------------------------------ | ---- |
| appkey      | 默认值：a5d25067-1f78-4a93-8060-6c6cb1492ef8                 | 必选 |
| guid        | 全局唯一GUID                                                 | 必选 |
| ltime       | 令牌时间戳                                                   | 必选 |
| online      |                                                              | 可选 |
| paperScores | 试卷分数                                                     | 必选 |
| pkey        | SsoUid、platformSource、version、time、type、guid、salt、token加密而成的密钥 | 必选 |
| platformSource | 来源平台 | 固定字符串“10” |
| time     | 13位字符串时间戳                          | 必选 |
| etime     | 13位字符串时间戳                          | 必选 |
| type        | 默认值：qz                                                   | 必选 |
| uid         | SsoUid                                                       | 必选 |
| userID         | SsoUid                                                       | 必选 |
| version     | 客户端版本号                                                 | 必选 |
| fordown   | 默认“1”                     | 必选  |

返回：

```json
{
    "success": true,
    "retry": true,
    "result": {
        "isNewQZ": 1,
        "code": "1",
        "finalList": [
            {
                "guid": "b477e529-f1e9-47ef-a160-cfcc2a3e8a1b",
                "result": [
                    {
                        "paperNewScoreID": 247320155,
                        "newQZSaveInfo": {
                            "isShowPC": 1,
                            "lastScore": 0,
                            "saveBizID": 28995054,
                            "doneScore": 1.5,
                            "synError": false,
                            "autoScore": 0,
                            "errorQuesNum": 0,
                            "bizKey": "/chinaacc/qz/36/centerPaper/mobile",
                            "isDoneText": 0,
                            "isShowApp": 1,
                            "examName": "第一部分 会计基础知识理论",
                            "doneQuesNum": 1,
                            "isTemp": 0,
                            "rightQuesNum": 0,
                            "userID": 33049216,
                            "autoRightNum": 0,
                            "bizID": 247320155,
                            "contestTimeLimit": 120,
                            "spendTime": 7,
                            "bizType": 2,
                            "allQuesNum": 1,
                            "autoNum": 1,
                            "undoneQuesNum": 0,
                            "recordDesc": "[2021年先学为快练习] 第一部分 会计基础知识理论"
                        },
                        "paperOldScoreID": "0"
                    }
                ],
                "code": "1"
            }
        ]
    }
}
```

返回数据包含如下字段：

| 字段      | 说明                       |
| --------- | -------------------------- |
| isNewQZ   |                            |
| finalList | StudentPaperFinalList集合  |
| code      | 结果代码，0为失败，1为成功 |

StudentPaperFinalList包含如下字段：

| 字段   | 说明                               |
| ------ | ---------------------------------- |
| guid   | 试卷的全局唯一ID（GUID），未用到   |
| result | StusentPaperResultFromSave集合     |
| code   | 结果代码，0为失败，1为成功，未用到 |

StudentPaperResultFromSave包含如下字段：

| 字段            | 说明   |
| --------------- | ------ |
| paperNewScoreID |        |
| paperOldScoreID | 未用到 |
| newQZSaveInfo   | 未用到 |

### 10608 从线上获取做题记录信息

> 通过该接口可以获取做题记录的信息。

资源地址：

```http
+/mapi/classroom/versionm/record/syncClouderPaperScoreBatchGateway
```

请求：

```json
{
    "params": "--加密串--",
    "domain": "chinaacc",
    "resourcePath": "+/mapi/classroom/versionm/record/syncClouderPaperScoreBatchGateway",
    "platform": "",
    "appType": "",
    "appKey": "",
    "appVersion": "",
    "publicKey": "--公钥--",
    "aesKey": "--加密串--",
    "sid": ""
}
```

需要传递如下参数:

| 字段          | 说明                                                         | 备注 |
| ------------- | ------------------------------------------------------------ | ---- |
| couseID       |                                                              | 可选 |
| ltime         | 令牌时间戳                                                   | 必选 |
| paperScoreIDs | 七天内做题记录ID                                             | 必选 |
| pkey          | SsoUid、platformSource、version、time、salt、token加密而成的密钥 | 必选 |
| platformSource | 来源平台 | 固定字符串“10” |
| time     | 13位字符串时间戳                          | 必选 |
| etime     | 13位字符串时间戳                          | 必选 |
| userID        | 用户ID                                                       | 必选 |
| version       | 客户端版本号                                                 | 必选 |

返回：

```json
{
    "success": true,
    "retry": true,
    "result": {
        "paperScores": [],
        "code": "1"
    }
}
```

result参数：

| 字段        | 说明                       |
| ----------- | -------------------------- |
| code        | 结果代码，0为失败，1为成功 |
| paperScores | StudentPaperScore集合      |

StudentPaperScore包含如下字段：

| 字段         | 说明                               |
| ------------ | ---------------------------------- |
| paperScoreID | 默认为0                            |
| paperScore   | 试卷总分                           |
| autoScore    | 试卷得分                           |
| lastScore    | 试卷得分                           |
| centerID     | 考试中心ID                         |
| userID       | 用户ID                             |
| siteCourseID | 对外课程ID                         |
| paperviewID  | 对外试卷ID                         |
| spendTime    | 做题用时                           |
| createTime   | 做题时间                           |
| answers      | StudentPaperAnswer（用户答案）集合 |

StudentPaperScoreAnswer（用户答案）包含如下字段：

| 字段         | 说明     |
| ------------ | -------- |
| paperScoreID | 默认为0  |
| questionID   | 题目ID   |
| userAnswer   | 用户答案 |
| userScore    | 用户得分 |
