...
{
    internal class MD5Algorithm : HashAlgorithm
    {
        ...
        {
            HashValue = GetHashFinalBlock(_data, 0, _dataSize, _abcd, _totalLength * 8);
            return HashValue;
        }
        ...
        private static uint R1(uint a, uint b, uint c, uint d, uint x, int s, uint t) => unchecked(b + Lsr(a + ((b & c) | ((b ^ 0xFFFFFFFF) & d)) + x + t, s));

        private static uint R2(uint a, uint b, uint c, uint d, uint x, int s, uint t) => unchecked(b + Lsr(a + ((b & d) | (c & (d ^ 0xFFFFFFFF))) + x + t, s));

        private static uint R3(uint a, uint b, uint c, uint d, uint x, int s, uint t) => unchecked(b + Lsr(a + (b ^ c ^ d) + x + t, s));

        private static uint R4(uint a, uint b, uint c, uint d, uint x, int s, uint t) => unchecked(b + Lsr(a + (c ^ (b | (d ^ 0xFFFFFFFF))) + x + t, s));
